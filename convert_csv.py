...
PERF_SCHEMA = StructType([
            StructField("account_number", LongType(), True),
            StructField("agent_id", StringType(), True),
            StructField("total_disk_bytes_read_per_sec_in_kbps", LongType(), True),
            StructField("total_disk_bytes_written_per_sec_in_kbps", LongType(), True),
            StructField("total_disk_read_ops_per_sec_in_kbps", LongType(), True),
            StructField("total_disk_write_ops_per_sec_in_kbps", LongType(), True),
            StructField("total_network_bytes_read_per_sec_in_kbps", LongType(), True),
            StructField("total_network_bytes_written_per_sec_in_kbps", LongType(), True),
            ...
            StructField("total_cpu_usage_pct", IntegerType(), True),
            StructField("total_disk_size_in_gb", LongType(), True),
            StructField("total_disk_free_size_in_gb", LongType(), True),
            StructField("total_ram_in_mb", LongType(), True),
            StructField("total_free_ram_in_mb", LongType(), True),
            StructField("timestamp", TimestampType(), True)])
...
def get_parquet_files(dir_path):
	...
	for agent in agent_dirs:
		...
		for export_type in agent_export_types:
			...
			for export in exports:
				# Remove colons if necessary from filename for compatability with Spark
	...
	for export_type in dfs:
		print("Converting " + export_type + " exports...")
		subfolder_dir = os.path.join(target_dir, export_type + ".parquet")
		...
		dfs[export_type].write.parquet(subfolder_dir)
